...
// IbanValidation SDK feature test
//
...
class FtCtrl extends IbanValidationControl
{
...
class FtClient
{
    ...
    private IbanValidationUtility $utility;

    public function __construct(IbanValidationUtility $utility, array $options)
    {
    ...
    public function get_utility(): IbanValidationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IbanValidationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IbanValidationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IbanValidationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IbanValidationConfig::make_feature($name);
    ...
    private static function buildUrl(IbanValidationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IbanValidationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IbanValidationResponse($fetched);
        }
    ...
    private function fail(IbanValidationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IbanValidationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
