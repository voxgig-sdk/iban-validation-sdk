...
// IbanValidation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IbanValidationOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IbanValidationOmni::structfix($now, Struct::undef()))
                        !== json_encode(IbanValidationOmni::structfix($val, Struct::undef()))) {
