...
// IbanValidation SDK pipeline test
//
...
require_once __DIR__ . '/../ibanvalidation_sdk.php';
...
{
    private static function utility(): IbanValidationUtility
    {
        return new IbanValidationUtility();
    }
    ...
    private static function ctx(array $over = []): IbanValidationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IbanValidationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IbanValidationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IbanValidationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IbanValidationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IbanValidationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IbanValidationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IbanValidationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IbanValidationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IbanValidationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IbanValidationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IbanValidationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = IbanValidationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IbanValidationSpec([]),
            'response' => null,
            'result' => new IbanValidationResult([]),
        ]);
        [, $err] = IbanValidationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IbanValidationSpec(['step' => 's']),
            'response' => new IbanValidationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IbanValidationResult([]),
        ]);
        [, $err] = IbanValidationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IbanValidationSpec(['step' => 's']),
            'response' => new IbanValidationResponse(self::resp(200, ['v' => 1])),
            'result' => new IbanValidationResult([]),
        ]);
        [, $err] = IbanValidationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IbanValidationSpec(['step' => 's']),
            'response' => new IbanValidationResponse(self::resp(200, ['v' => 2])),
            'result' => new IbanValidationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IbanValidationMakeResponse::call($ctx);
    ...
    {
        $preset = new IbanValidationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IbanValidationSpec([]),
            'response' => new IbanValidationResponse([]),
            'result' => new IbanValidationResult([]),
        ...
        [$got, $err] = IbanValidationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IbanValidationResult([])]);
        [, $err] = IbanValidationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IbanValidationSpec([]), 'result' => null]);
        [, $err] = IbanValidationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IbanValidationSpec(['step' => 's']),
            'result' => new IbanValidationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IbanValidationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IbanValidationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IbanValidationSpec(['step' => 's']),
            'result' => new IbanValidationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IbanValidationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IbanValidationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IbanValidationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IbanValidationSpec([]),
            'result' => new IbanValidationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IbanValidationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IbanValidationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IbanValidationError('boom', 'boom');
        $utility->fetcher = function (IbanValidationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IbanValidationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IbanValidationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IbanValidationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IbanValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IbanValidationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        IbanValidationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IbanValidationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IbanValidationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IbanValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IbanValidationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IbanValidationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IbanValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IbanValidationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IbanValidationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IbanValidationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IbanValidationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IbanValidationResult(['ok' => false])]);
        $this->expectException(IbanValidationError::class);
        IbanValidationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IbanValidationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IbanValidationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IbanValidationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IbanValidationBaseFeature();
        $b = new IbanValidationBaseFeature();
        IbanValidationFeatureAdd::call($ctx, $a);
        IbanValidationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IbanValidationBaseFeature
    {
        $f = new IbanValidationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IbanValidationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IbanValidationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IbanValidationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IbanValidationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        IbanValidationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IbanValidationResponse(['headers' => null]),
            'result' => new IbanValidationResult([]),
        ]);
        IbanValidationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        IbanValidationResultBody::call($ctx);
