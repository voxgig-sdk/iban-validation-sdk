...
boru-driven command-line client **and** interactive REPL for the IbanValidation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iban-validation-cli)
make build

# 2. See usage (words, entities, env vars)
./iban-validation-cli --help

# 3. Provide credentials once, via the environment
export IBAN_VALIDATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./iban-validation-cli load 1 dns_result            # {id:1} shorthand
./iban-validation-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
IBAN_VALIDATION_BASE=https://api.example.com ./iban-validation-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./iban-validation-cli
iban-validation> load 1 dns_result
iban-validation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iban-validation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/iban-validation-cli load 1 dns_result
   ```
...
```sh
./iban-validation-cli load 1 dns_result          # scalar shorthand for {id:1}
./iban-validation-cli load '{id:1}' dns_result     # explicit match map
```
...
./iban-validation-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `iban-validation>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/iban-validation-cli`. |
...
(`./iban-validation-cli <expr>`) and interactively (the REPL), and expressions compose the
