...
const __1 = require("..");
const SDK_NAME = 'IbanValidationSDK';
...
function isRunnable(code) {
    return (/new\s+IbanValidationSDK\b/.test(code) ||
        /\bIbanValidationSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IbanValidationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IbanValidationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
