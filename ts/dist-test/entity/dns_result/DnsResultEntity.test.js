...
(0, node_test_1.describe)('DnsResultEntity', async () => {
    ...
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('IBAN_VALIDATION_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.IbanValidationSDK.test();
        const ent = testsdk.DnsResult();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.IBAN_VALIDATION_TEST_LIVE;
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set IBAN_VALIDATION_TEST_DNS_RESULT_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.IbanValidationSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['IBAN_VALIDATION_TEST_DNS_RESULT_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'IBAN_VALIDATION_TEST_DNS_RESULT_ENTID': idmap,
        'IBAN_VALIDATION_TEST_LIVE': 'FALSE',
        'IBAN_VALIDATION_TEST_EXPLAIN': 'FALSE',
        'IBAN_VALIDATION_APIKEY': '',
    });
    idmap = env['IBAN_VALIDATION_TEST_DNS_RESULT_ENTID'];
    const live = 'TRUE' === env.IBAN_VALIDATION_TEST_LIVE;
    if (live) {
        client = new __1.IbanValidationSDK(merge([
    ...
    const setup = {
        ...
        struct,
        data: entityData,
        explain: 'TRUE' === env.IBAN_VALIDATION_TEST_EXPLAIN,
        live,
