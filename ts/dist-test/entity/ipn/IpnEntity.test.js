...
(0, node_test_1.describe)('IpnEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set IBAN_VALIDATION_TEST_IPN_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    const idmapEnvVal = process.env['IBAN_VALIDATION_TEST_IPN_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'IBAN_VALIDATION_TEST_IPN_ENTID': idmap,
        'IBAN_VALIDATION_TEST_LIVE': 'FALSE',
        'IBAN_VALIDATION_TEST_EXPLAIN': 'FALSE',
        'IBAN_VALIDATION_APIKEY': '',
    });
    idmap = env['IBAN_VALIDATION_TEST_IPN_ENTID'];
