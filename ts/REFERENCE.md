# IbanValidation TypeScript SDK Reference

Complete API reference for the IbanValidation TypeScript SDK.
...
```ts
new IbanValidationSDK(options?: object)
```
...
#### `IbanValidationSDK.test(testopts?, sdkopts?)`
...
**Returns:** `IbanValidationSDK` instance in test mode.
...
Alias for `IbanValidationSDK.test()`.
...
Return the parent `IbanValidationSDK` instance.
