# IbanValidation Ruby SDK



The Ruby SDK for the IbanValidation API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "IbanValidation_sdk"
client = IbanValidationSDK.new(options)
```
...
```ruby
client = IbanValidationSDK.test(testopts, sdkopts)
```
...
raise a `IbanValidationError` (a `StandardError` subclass), so wrap
...
```
rb/
├── IbanValidation_sdk.rb       -- Main SDK module
...
The main module (`IbanValidation_sdk`) exports the SDK class
