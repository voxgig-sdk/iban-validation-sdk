...
require_relative 'IbanValidation_types'
...
class IbanValidationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IbanValidationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IbanValidationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IbanValidationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IbanValidationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IbanValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IbanValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IbanValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue IbanValidationError => err
    ...
    if fetched.is_a?(Hash)
      status = IbanValidationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IbanValidationError.new(
        "graphql_error", "IbanValidationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IbanValidationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
