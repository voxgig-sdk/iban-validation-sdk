...
module IbanValidationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IbanValidationResponse.new({})
    ...
    if fetch_err
      ...
      response = IbanValidationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IbanValidationResponse.new(fetched)
    else
