...
class DomainEntityTest < Minitest::Test
  def test_create_instance
    testsdk = IbanValidationSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = IbanValidationSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IBAN_VALIDATION_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["IBAN_VALIDATION_TEST_DOMAIN_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["IBAN_VALIDATION_TEST_DOMAIN_ENTID"])
  ...
  if env["IBAN_VALIDATION_TEST_LIVE"] == "TRUE"
    ...
    client = IbanValidationSDK.new(Helpers.to_map(merged_opts))
  end
