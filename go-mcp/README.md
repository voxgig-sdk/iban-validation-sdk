...
[MCP](https://modelcontextprotocol.io) server exposing the IbanValidation SDK as
two agent tools — `iban-validation_list` and `iban-validation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iban-validation-mcp)
make build

# 2. Provide credentials via the environment
export IBAN_VALIDATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user iban-validation \
  -- /absolute/path/to/iban-validation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./iban-validation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// iban-validation_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// iban-validation_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iban-validation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user iban-validation \
     -- "$PWD"/dist/*/iban-validation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `iban-validation_list` and `iban-validation_load` tools now appear
   in new sessions. Ask the agent to *"list domain using iban-validation"*
   and it calls `iban-validation_list` with `{"entity":"domain"}`.
...
```sh
export IBAN_VALIDATION_APIKEY=sk_live_xxx            # API key
export IBAN_VALIDATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `iban-validation_list` tool
...
### Call the `iban-validation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `iban-validation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `iban-validation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IBAN_VALIDATION_APIKEY` | API key sent with every request. |
| `IBAN_VALIDATION_BASE` | Optional override of the API base URL. |
...
```sh
./iban-validation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"iban-validation_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
