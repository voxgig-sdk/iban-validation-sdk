...
module IbanValidationUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IbanValidationContext.new({}, nil)
    end
    op = ctx.op || IbanValidationOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IbanValidationResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IbanValidationError) ? err.msg : err.to_s
    ...
    sdk_err = IbanValidationError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(IbanValidationError)
