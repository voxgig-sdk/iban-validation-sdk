...
// IbanValidation SDK retry feature
...
class IbanValidationRetryFeature extends IbanValidationBaseFeature
{
    ...
    public function init(IbanValidationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IbanValidationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IbanValidationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IbanValidationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
