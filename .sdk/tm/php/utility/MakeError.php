...
class IbanValidationMakeError
{
    public static function call(?IbanValidationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IbanValidationContext([], null);
        }
        $op = $ctx->op ?? new IbanValidationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IbanValidationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IbanValidationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IbanValidationError('', $msg, $ctx);
