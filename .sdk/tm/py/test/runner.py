# IbanValidation SDK test runner
...
class IbanValidationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IbanValidationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IbanValidationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IbanValidationTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = IbanValidationTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IbanValidationTestRunner.getenv(key)
        ...
        explain = IbanValidationTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IbanValidationTestRunner._test_control is not None:
            return IbanValidationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IbanValidationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IbanValidationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IbanValidationTestRunner.load_test_control()
...
def load_env_local():
    IbanValidationTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return IbanValidationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IbanValidationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IbanValidationTestRunner.load_test_control()


def live_client_options():
    return IbanValidationTestRunner.live_client_options()


def live_delay_ms():
    return IbanValidationTestRunner.live_delay_ms()
