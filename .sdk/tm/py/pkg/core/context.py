...
from projectname_sdk.core.control import IbanValidationControl
...
class IbanValidationContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = IbanValidationControl()
        ...
        if isinstance(sp, IbanValidationSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, IbanValidationResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, IbanValidationResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return IbanValidationOperation({})
    ...
    def make_error(self, code, msg):
        return IbanValidationError(code, msg, self)
