...
import (
	...
	omni "github.com/voxgig-sdk/iban-validation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IbanValidationError); is {
				out := map[string]any{
					"name":    "IbanValidationError",
...
func omniCtx(arg any, client *sdk.IbanValidationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IbanValidationSDK); is {
			client = live
