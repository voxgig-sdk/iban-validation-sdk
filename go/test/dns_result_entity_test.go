...
func TestDnsResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IBANVALIDATION_TEST_DNS_RESULT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func dns_resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IBANVALIDATION_TEST_DNS_RESULT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"IBANVALIDATION_TEST_DNS_RESULT_ENTID": idmap,
		"IBANVALIDATION_TEST_LIVE":      "FALSE",
		"IBANVALIDATION_TEST_EXPLAIN":   "FALSE",
		"IBANVALIDATION_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["IBANVALIDATION_TEST_DNS_RESULT_ENTID"])
