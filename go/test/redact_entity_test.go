...
func TestRedactEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IBANVALIDATION_TEST_REDACT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func redactBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IBANVALIDATION_TEST_REDACT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"IBANVALIDATION_TEST_REDACT_ENTID": idmap,
		"IBANVALIDATION_TEST_LIVE":      "FALSE",
		"IBANVALIDATION_TEST_EXPLAIN":   "FALSE",
		"IBANVALIDATION_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["IBANVALIDATION_TEST_REDACT_ENTID"])
